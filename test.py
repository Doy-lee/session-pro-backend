...
def test_backend_same_user_stacks_subscription():
    ...
    for it in scenarios:
        ...
        add_payment_hash: bytes = backend.make_payment_hash(master_pkey=master_key.verify_key,
                                                            rotating_pkey=rotating_key.verify_key,
                                                            payment_token_hash=it.payment_token_hash)

        it.proof = backend.add_payment(sql_conn           = db.sql_conn,
...
def test_server():
    ...
    payment_hash_to_sign: bytes = backend.make_payment_hash(master_pkey=master_key.verify_key,
                                                            rotating_pkey=rotating_key.verify_key,
                                                            payment_token_hash=payment_token_hash)

    flask_client: werkzeug.Client            = flask_app.test_client()
    response:     werkzeug.test.TestResponse = flask_client.post("/add_payment", json={
        'version':       0,
    ...
    # Parse the JSON from the respones
    ...
    db.runtime.backend_key.verify_key.verify(smessage=proof_hash, signature=result_sig)
