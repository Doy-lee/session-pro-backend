...
def test_platform_apple():
    ...
    if 1: # Did renew notification
        ...
        err = base.ErrorSink()
        platform_apple.handle_notification(decoded_notification=notification, sql_conn=db.sql_conn, err=err)
    ...
    if 1:
        if 1: # Subscribe notification
            ...
            err = base.ErrorSink()
            platform_apple.handle_notification(decoded_notification=decoded_notification, sql_conn=db.sql_conn, err=err)
            assert not err.has(), err.msg_list
        ...
        if 1: # Did change renewal status notification
            ...
            assert payment_list[1].apple.tx_id                       == tx_info.transactionId
            assert payment_list[1].apple.web_line_order_tx_id        == tx_info.webOrderLineItemId
        ...
        if 1: # Expire (voluntary) notification
            ...
            # NOTE: Check the original payment will also expire if we advance to payment 1's
            # expiry date
            assert payment_list[0].expiry_unix_ts_ms <= payment_list[1].expiry_unix_ts_ms

            # NOTE: Now expire the payment
            backend.expire_payments_revocations_and_users(sql_conn=db.sql_conn, unix_ts_ms=payment_list[1].expiry_unix_ts_ms + 1)

            # NOTE: Now check that the payments were marked expired
            payment_list: list[backend.PaymentRow] = backend.get_payments_list(db.sql_conn)
            assert len(payment_list) == 2
            assert payment_list[0].status == backend.PaymentStatus.Expired

            assert payment_list[1].master_pkey                       == None
            assert payment_list[1].status                            == backend.PaymentStatus.Expired
            assert payment_list[1].plan                              == backend.ProPlanType.OneMonth
            assert payment_list[1].payment_provider                  == base.PaymentProvider.iOSAppStore
            assert payment_list[1].unredeemed_unix_ts_ms             == tx_info.purchaseDate
            assert payment_list[1].redeemed_unix_ts_ms               == None
    ...
    if 1:
        ...
        if 0:
            ...
            e01_upgrade_to_3months: dict[str, base.JSONValue] = json.loads('''
            {
            ...
            e03_downgrade_to_1wk: dict[str, base.JSONValue] = json.loads('''
            {
            ...
            e04_revert_to_3months: dict[str, base.JSONValue] = json.loads('''
            {
            ...
            e00_sub_to_1wk_signed_payload:         str = typing.cast(str, e00_sub_to_1wk['signedPayload'])
            e01_upgrade_to_3months_signed_payload: str = typing.cast(str, e01_upgrade_to_3months['signedPayload'])
            e02_disable_auto_renew_signed_payload: str = typing.cast(str, e02_disable_auto_renew['signedPayload'])
            e03_downgrade_to_1wk_signed_payload:   str = typing.cast(str, e03_downgrade_to_1wk['signedPayload'])
            e04_revert_to_3months_signed_payload:  str = typing.cast(str, e04_revert_to_3months['signedPayload'])
            ...
            e00_sub_to_1wk_decoded_body:         AppleResponseBodyV2DecodedPayload = core.signed_data_verifier.verify_and_decode_notification(e00_sub_to_1wk_signed_payload)
            e01_upgrade_to_3months_decoded_body: AppleResponseBodyV2DecodedPayload = core.signed_data_verifier.verify_and_decode_notification(e01_upgrade_to_3months_signed_payload)
            ...
            e03_downgrade_to_1wk_decoded_body:   AppleResponseBodyV2DecodedPayload = core.signed_data_verifier.verify_and_decode_notification(e03_downgrade_to_1wk_signed_payload)
            e04_revert_to_3months_decoded_body:  AppleResponseBodyV2DecodedPayload = core.signed_data_verifier.verify_and_decode_notification(e04_revert_to_3months_signed_payload)
            ...
            dump_apple_signed_payloads(core, e00_sub_to_1wk_decoded_body,         'e00_sub_to_1wk_')
            dump_apple_signed_payloads(core, e01_upgrade_to_3months_decoded_body, 'e01_upgrade_to_3months_')
            dump_apple_signed_payloads(core, e02_disable_auto_renew_decoded_body, 'e02_disable_auto_renew_')
            dump_apple_signed_payloads(core, e03_downgrade_to_1wk_decoded_body,   'e03_downgrade_to_1wk_')
            dump_apple_signed_payloads(core, e04_revert_to_3months_decoded_body,  'e04_revert_to_3months_')
        ...
        # NOTE: Generated by dump_apple_signed_payloads
        # NOTE: Signed Payload
        e00_sub_to_1wk_body                                             = AppleResponseBodyV2DecodedPayload()
        e00_sub_to_1wk_body_data                                        = AppleData()
        e00_sub_to_1wk_body_data.appAppleId                             = 1470168868
        e00_sub_to_1wk_body_data.bundleId                               = 'com.loki-project.loki-messenger'
        e00_sub_to_1wk_body_data.bundleVersion                          = '637'
        e00_sub_to_1wk_body_data.consumptionRequestReason               = None
        e00_sub_to_1wk_body_data.environment                            = AppleEnvironment.SANDBOX
        e00_sub_to_1wk_body_data.rawConsumptionRequestReason            = None
        e00_sub_to_1wk_body_data.rawEnvironment                         = 'Sandbox'
        e00_sub_to_1wk_body_data.rawStatus                              = 1
        e00_sub_to_1wk_body_data.signedRenewalInfo                      = '..s3yvWeyKDVbZSciqRVRpMBGkpNnQ10lEn-6jXL9yPK-Yi-nEkXOLIzl4Ji5VutI-2kY2vZlxj7mVXu-2v3Mi0Q'
        e00_sub_to_1wk_body_data.signedTransactionInfo                  = '..bMfO0cWYCajFTg5Mmv2nyU_lBQfyPM9Z5pOw_B6jO9jy1qxc49DffTVf_ZOU5HH-04W7t3qczVi8KkLeWNbi_A'
        e00_sub_to_1wk_body_data.status                                 = AppleStatus.ACTIVE
        e00_sub_to_1wk_body.data                                        = e00_sub_to_1wk_body_data
        e00_sub_to_1wk_body.externalPurchaseToken                       = None
        e00_sub_to_1wk_body.notificationType                            = AppleNotificationTypeV2.SUBSCRIBED
        e00_sub_to_1wk_body.notificationUUID                            = 'fee6ade6-5871-4e0f-9d2e-5d6229a24027'
        e00_sub_to_1wk_body.rawNotificationType                         = 'SUBSCRIBED'
        e00_sub_to_1wk_body.rawSubtype                                  = 'RESUBSCRIBE'
        e00_sub_to_1wk_body.signedDate                                  = 1759727785445
        e00_sub_to_1wk_body.subtype                                     = AppleSubtype.RESUBSCRIBE
        e00_sub_to_1wk_body.summary                                     = None
        e00_sub_to_1wk_body.version                                     = '2.0'

        # NOTE: Signed Renewal Info
        e00_sub_to_1wk_renewal_info                                     = AppleJWSRenewalInfoDecodedPayload()
        e00_sub_to_1wk_renewal_info.appAccountToken                     = None
        e00_sub_to_1wk_renewal_info.appTransactionId                    = '704897469903383919'
        e00_sub_to_1wk_renewal_info.autoRenewProductId                  = None
        e00_sub_to_1wk_renewal_info.autoRenewStatus                     = None
        e00_sub_to_1wk_renewal_info.currency                            = 'AUD'
        e00_sub_to_1wk_renewal_info.eligibleWinBackOfferIds             = None
        e00_sub_to_1wk_renewal_info.environment                         = AppleEnvironment.SANDBOX
        e00_sub_to_1wk_renewal_info.expirationIntent                    = None
        e00_sub_to_1wk_renewal_info.gracePeriodExpiresDate              = None
        e00_sub_to_1wk_renewal_info.isInBillingRetryPeriod              = None
        e00_sub_to_1wk_renewal_info.offerDiscountType                   = None
        e00_sub_to_1wk_renewal_info.offerIdentifier                     = None
        e00_sub_to_1wk_renewal_info.offerPeriod                         = None
        e00_sub_to_1wk_renewal_info.offerType                           = None
        e00_sub_to_1wk_renewal_info.originalTransactionId               = '2000001024993299'
        e00_sub_to_1wk_renewal_info.priceIncreaseStatus                 = None
        e00_sub_to_1wk_renewal_info.productId                           = 'com.getsession.org.pro_sub_3_months'
        e00_sub_to_1wk_renewal_info.rawAutoRenewStatus                  = None
        e00_sub_to_1wk_renewal_info.rawEnvironment                      = 'Sandbox'
        e00_sub_to_1wk_renewal_info.rawExpirationIntent                 = None
        e00_sub_to_1wk_renewal_info.rawOfferDiscountType                = None
        e00_sub_to_1wk_renewal_info.rawOfferType                        = None
        e00_sub_to_1wk_renewal_info.rawPriceIncreaseStatus              = None
        e00_sub_to_1wk_renewal_info.recentSubscriptionStartDate         = None
        e00_sub_to_1wk_renewal_info.renewalDate                         = None
        e00_sub_to_1wk_renewal_info.renewalPrice                        = None
        e00_sub_to_1wk_renewal_info.signedDate                          = 1759727785445

        # NOTE: Signed Transaction Info
        e00_sub_to_1wk_tx_info                                          = AppleJWSTransactionDecodedPayload()
        e00_sub_to_1wk_tx_info.appAccountToken                          = None
        e00_sub_to_1wk_tx_info.appTransactionId                         = '704897469903383919'
        e00_sub_to_1wk_tx_info.bundleId                                 = 'com.loki-project.loki-messenger'
        e00_sub_to_1wk_tx_info.currency                                 = 'AUD'
        e00_sub_to_1wk_tx_info.environment                              = AppleEnvironment.SANDBOX
        e00_sub_to_1wk_tx_info.expiresDate                              = 1759728318000
        e00_sub_to_1wk_tx_info.inAppOwnershipType                       = AppleInAppOwnershipType.PURCHASED
        e00_sub_to_1wk_tx_info.isUpgraded                               = None
        e00_sub_to_1wk_tx_info.offerDiscountType                        = None
        e00_sub_to_1wk_tx_info.offerIdentifier                          = None
        e00_sub_to_1wk_tx_info.offerPeriod                              = None
        e00_sub_to_1wk_tx_info.offerType                                = None
        e00_sub_to_1wk_tx_info.originalPurchaseDate                     = 1759301833000
        e00_sub_to_1wk_tx_info.originalTransactionId                    = '2000001024993299'
        e00_sub_to_1wk_tx_info.price                                    = 5990
        e00_sub_to_1wk_tx_info.productId                                = 'com.getsession.org.pro_sub_3_months'
        e00_sub_to_1wk_tx_info.purchaseDate                             = 1759727778000
        e00_sub_to_1wk_tx_info.quantity                                 = 1
        e00_sub_to_1wk_tx_info.rawEnvironment                           = 'Sandbox'
        e00_sub_to_1wk_tx_info.rawInAppOwnershipType                    = 'PURCHASED'
        e00_sub_to_1wk_tx_info.rawOfferDiscountType                     = None
        e00_sub_to_1wk_tx_info.rawOfferType                             = None
        e00_sub_to_1wk_tx_info.rawRevocationReason                      = None
        e00_sub_to_1wk_tx_info.rawTransactionReason                     = 'PURCHASE'
        e00_sub_to_1wk_tx_info.rawType                                  = 'Auto-Renewable Subscription'
        e00_sub_to_1wk_tx_info.revocationDate                           = None
        e00_sub_to_1wk_tx_info.revocationReason                         = None
        e00_sub_to_1wk_tx_info.signedDate                               = 1759727785445
        e00_sub_to_1wk_tx_info.storefront                               = 'AUS'
        e00_sub_to_1wk_tx_info.storefrontId                             = '143460'
        e00_sub_to_1wk_tx_info.subscriptionGroupIdentifier              = '21752814'
        e00_sub_to_1wk_tx_info.transactionId                            = '2000001027701898'
        e00_sub_to_1wk_tx_info.transactionReason                        = AppleTransactionReason.PURCHASE
        e00_sub_to_1wk_tx_info.type                                     = AppleType.AUTO_RENEWABLE_SUBSCRIPTION
        e00_sub_to_1wk_tx_info.webOrderLineItemId                       = '2000000113864605'

        e00_sub_to_1wk_decoded_notification                             = platform_apple.DecodedNotification(body=e00_sub_to_1wk_body, tx_info=e00_sub_to_1wk_tx_info, renewal_info=e00_sub_to_1wk_renewal_info)

        # NOTE: Generated by dump_apple_signed_payloads
        # NOTE: Signed Payload
        e01_upgrade_to_3months_body                                     = AppleResponseBodyV2DecodedPayload()
        e01_upgrade_to_3months_body_data                                = AppleData()
        e01_upgrade_to_3months_body_data.appAppleId                     = 1470168868
        e01_upgrade_to_3months_body_data.bundleId                       = 'com.loki-project.loki-messenger'
        e01_upgrade_to_3months_body_data.bundleVersion                  = '637'
        e01_upgrade_to_3months_body_data.consumptionRequestReason       = None
        e01_upgrade_to_3months_body_data.environment                    = AppleEnvironment.SANDBOX
        e01_upgrade_to_3months_body_data.rawConsumptionRequestReason    = None
        e01_upgrade_to_3months_body_data.rawEnvironment                 = 'Sandbox'
        e01_upgrade_to_3months_body_data.rawStatus                      = 1
        e01_upgrade_to_3months_body_data.signedRenewalInfo              = '..-iX0StcW4lxx8zCskl1SFf-HNOV5KiwddKJ42XmWaFCsEuABtAWszBEsJu4OIQTHh6aamYx5CkoBTeUy6hsRtg'
        e01_upgrade_to_3months_body_data.signedTransactionInfo          = '..PHyVvDlmjoO3KFj9U_JsA7pxIp730GIwIskog5Pfsrcb_HZpXcU1LUOZYkQJjtfVPyfunMpIJAuomhfcXiGk1g'
        e01_upgrade_to_3months_body_data.status                         = AppleStatus.ACTIVE
        e01_upgrade_to_3months_body.data                                = e01_upgrade_to_3months_body_data
        e01_upgrade_to_3months_body.externalPurchaseToken               = None
        e01_upgrade_to_3months_body.notificationType                    = AppleNotificationTypeV2.DID_CHANGE_RENEWAL_PREF
        e01_upgrade_to_3months_body.notificationUUID                    = 'a3a3b7ae-3bd8-4b98-a83d-e5f4380aeaa2'
        e01_upgrade_to_3months_body.rawNotificationType                 = 'DID_CHANGE_RENEWAL_PREF'
        e01_upgrade_to_3months_body.rawSubtype                          = 'UPGRADE'
        e01_upgrade_to_3months_body.signedDate                          = 1759727799810
        e01_upgrade_to_3months_body.subtype                             = AppleSubtype.UPGRADE
        e01_upgrade_to_3months_body.summary                             = None
        e01_upgrade_to_3months_body.version                             = '2.0'

        # NOTE: Signed Renewal Info
        e01_upgrade_to_3months_renewal_info                             = AppleJWSRenewalInfoDecodedPayload()
        e01_upgrade_to_3months_renewal_info.appAccountToken             = None
        e01_upgrade_to_3months_renewal_info.appTransactionId            = '704897469903383919'
        e01_upgrade_to_3months_renewal_info.autoRenewProductId          = None
        e01_upgrade_to_3months_renewal_info.autoRenewStatus             = None
        e01_upgrade_to_3months_renewal_info.currency                    = 'AUD'
        e01_upgrade_to_3months_renewal_info.eligibleWinBackOfferIds     = None
        e01_upgrade_to_3months_renewal_info.environment                 = AppleEnvironment.SANDBOX
        e01_upgrade_to_3months_renewal_info.expirationIntent            = None
        e01_upgrade_to_3months_renewal_info.gracePeriodExpiresDate      = None
        e01_upgrade_to_3months_renewal_info.isInBillingRetryPeriod      = None
        e01_upgrade_to_3months_renewal_info.offerDiscountType           = None
        e01_upgrade_to_3months_renewal_info.offerIdentifier             = None
        e01_upgrade_to_3months_renewal_info.offerPeriod                 = None
        e01_upgrade_to_3months_renewal_info.offerType                   = None
        e01_upgrade_to_3months_renewal_info.originalTransactionId       = '2000001024993299'
        e01_upgrade_to_3months_renewal_info.priceIncreaseStatus         = None
        e01_upgrade_to_3months_renewal_info.productId                   = 'com.getsession.org.pro_sub'
        e01_upgrade_to_3months_renewal_info.rawAutoRenewStatus          = None
        e01_upgrade_to_3months_renewal_info.rawEnvironment              = 'Sandbox'
        e01_upgrade_to_3months_renewal_info.rawExpirationIntent         = None
        e01_upgrade_to_3months_renewal_info.rawOfferDiscountType        = None
        e01_upgrade_to_3months_renewal_info.rawOfferType                = None
        e01_upgrade_to_3months_renewal_info.rawPriceIncreaseStatus      = None
        e01_upgrade_to_3months_renewal_info.recentSubscriptionStartDate = None
        e01_upgrade_to_3months_renewal_info.renewalDate                 = None
        e01_upgrade_to_3months_renewal_info.renewalPrice                = None
        e01_upgrade_to_3months_renewal_info.signedDate                  = 1759727799810

        # NOTE: Signed Transaction Info
        e01_upgrade_to_3months_tx_info                                  = AppleJWSTransactionDecodedPayload()
        e01_upgrade_to_3months_tx_info.appAccountToken                  = None
        e01_upgrade_to_3months_tx_info.appTransactionId                 = '704897469903383919'
        e01_upgrade_to_3months_tx_info.bundleId                         = 'com.loki-project.loki-messenger'
        e01_upgrade_to_3months_tx_info.currency                         = 'AUD'
        e01_upgrade_to_3months_tx_info.environment                      = AppleEnvironment.SANDBOX
        e01_upgrade_to_3months_tx_info.expiresDate                      = 1759727970000
        e01_upgrade_to_3months_tx_info.inAppOwnershipType               = AppleInAppOwnershipType.PURCHASED
        e01_upgrade_to_3months_tx_info.isUpgraded                       = None
        e01_upgrade_to_3months_tx_info.offerDiscountType                = None
        e01_upgrade_to_3months_tx_info.offerIdentifier                  = None
        e01_upgrade_to_3months_tx_info.offerPeriod                      = None
        e01_upgrade_to_3months_tx_info.offerType                        = None
        e01_upgrade_to_3months_tx_info.originalPurchaseDate             = 1759301833000
        e01_upgrade_to_3months_tx_info.originalTransactionId            = '2000001024993299'
        e01_upgrade_to_3months_tx_info.price                            = 1990
        e01_upgrade_to_3months_tx_info.productId                        = 'com.getsession.org.pro_sub'
        e01_upgrade_to_3months_tx_info.purchaseDate                     = 1759727790000
        e01_upgrade_to_3months_tx_info.quantity                         = 1
        e01_upgrade_to_3months_tx_info.rawEnvironment                   = 'Sandbox'
        e01_upgrade_to_3months_tx_info.rawInAppOwnershipType            = 'PURCHASED'
        e01_upgrade_to_3months_tx_info.rawOfferDiscountType             = None
        e01_upgrade_to_3months_tx_info.rawOfferType                     = None
        e01_upgrade_to_3months_tx_info.rawRevocationReason              = None
        e01_upgrade_to_3months_tx_info.rawTransactionReason             = 'PURCHASE'
        e01_upgrade_to_3months_tx_info.rawType                          = 'Auto-Renewable Subscription'
        e01_upgrade_to_3months_tx_info.revocationDate                   = None
        e01_upgrade_to_3months_tx_info.revocationReason                 = None
        e01_upgrade_to_3months_tx_info.signedDate                       = 1759727799810
        e01_upgrade_to_3months_tx_info.storefront                       = 'AUS'
        e01_upgrade_to_3months_tx_info.storefrontId                     = '143460'
        e01_upgrade_to_3months_tx_info.subscriptionGroupIdentifier      = '21752814'
        e01_upgrade_to_3months_tx_info.transactionId                    = '2000001027701928'
        e01_upgrade_to_3months_tx_info.transactionReason                = AppleTransactionReason.PURCHASE
        e01_upgrade_to_3months_tx_info.type                             = AppleType.AUTO_RENEWABLE_SUBSCRIPTION
        e01_upgrade_to_3months_tx_info.webOrderLineItemId               = '2000000114202140'

        e01_upgrade_to_3months_decoded_notification                     = platform_apple.DecodedNotification(body=e01_upgrade_to_3months_body, tx_info=e01_upgrade_to_3months_tx_info, renewal_info=e01_upgrade_to_3months_renewal_info)
        ...
        # NOTE: Generated by dump_apple_signed_payloads
        # NOTE: Signed Payload
        e03_downgrade_to_1wk_body                                       = AppleResponseBodyV2DecodedPayload()
        e03_downgrade_to_1wk_body_data                                  = AppleData()
        e03_downgrade_to_1wk_body_data.appAppleId                       = 1470168868
        e03_downgrade_to_1wk_body_data.bundleId                         = 'com.loki-project.loki-messenger'
        e03_downgrade_to_1wk_body_data.bundleVersion                    = '637'
        e03_downgrade_to_1wk_body_data.consumptionRequestReason         = None
        e03_downgrade_to_1wk_body_data.environment                      = AppleEnvironment.SANDBOX
        e03_downgrade_to_1wk_body_data.rawConsumptionRequestReason      = None
        e03_downgrade_to_1wk_body_data.rawEnvironment                   = 'Sandbox'
        e03_downgrade_to_1wk_body_data.rawStatus                        = 1
        e03_downgrade_to_1wk_body_data.signedRenewalInfo                = '..x9ex7aFlmHJ3U286ZLPluVX5ES0PjgVzIVOxxzKVjrNjRi-GVsSHuDSJbz_wkF0zBBtvEcOOgOnjIqiHVRPmMA'
        e03_downgrade_to_1wk_body_data.signedTransactionInfo               = '..CjL3XeQfKQaz-bCwrYsDGlVKTHgoeg1zXXsheGtsuMsqzxJrjF7VdutsywY_YyQ7lzWiOuEB4nqXX96ZvJpJiA'
        e03_downgrade_to_1wk_body_data.status                           = AppleStatus.ACTIVE
        e03_downgrade_to_1wk_body.data                                  = e03_downgrade_to_1wk_body_data
        e03_downgrade_to_1wk_body.externalPurchaseToken                 = None
        e03_downgrade_to_1wk_body.notificationType                      = AppleNotificationTypeV2.DID_CHANGE_RENEWAL_PREF
        e03_downgrade_to_1wk_body.notificationUUID                      = '01c574dc-64b7-45d3-889a-5fdcb08fb5c0'
        e03_downgrade_to_1wk_body.rawNotificationType                   = 'DID_CHANGE_RENEWAL_PREF'
        e03_downgrade_to_1wk_body.rawSubtype                            = 'DOWNGRADE'
        e03_downgrade_to_1wk_body.signedDate                            = 1759727838450
        e03_downgrade_to_1wk_body.subtype                               = AppleSubtype.DOWNGRADE
        e03_downgrade_to_1wk_body.summary                               = None
        e03_downgrade_to_1wk_body.version                               = '2.0'

        # NOTE: Signed Renewal Info
        e03_downgrade_to_1wk_renewal_info                               = AppleJWSRenewalInfoDecodedPayload()
        e03_downgrade_to_1wk_renewal_info.appAccountToken               = None
        e03_downgrade_to_1wk_renewal_info.appTransactionId              = '704897469903383919'
        e03_downgrade_to_1wk_renewal_info.autoRenewProductId            = None
        e03_downgrade_to_1wk_renewal_info.autoRenewStatus               = None
        e03_downgrade_to_1wk_renewal_info.currency                      = 'AUD'
        e03_downgrade_to_1wk_renewal_info.eligibleWinBackOfferIds       = None
        e03_downgrade_to_1wk_renewal_info.environment                   = AppleEnvironment.SANDBOX
        e03_downgrade_to_1wk_renewal_info.expirationIntent              = None
        e03_downgrade_to_1wk_renewal_info.gracePeriodExpiresDate        = None
        e03_downgrade_to_1wk_renewal_info.isInBillingRetryPeriod        = None
        e03_downgrade_to_1wk_renewal_info.offerDiscountType             = None
        e03_downgrade_to_1wk_renewal_info.offerIdentifier               = None
        e03_downgrade_to_1wk_renewal_info.offerPeriod                   = None
        e03_downgrade_to_1wk_renewal_info.offerType                     = None
        e03_downgrade_to_1wk_renewal_info.originalTransactionId         = '2000001024993299'
        e03_downgrade_to_1wk_renewal_info.priceIncreaseStatus           = None
        e03_downgrade_to_1wk_renewal_info.productId                     = 'com.getsession.org.pro_sub'
        e03_downgrade_to_1wk_renewal_info.rawAutoRenewStatus            = None
        e03_downgrade_to_1wk_renewal_info.rawEnvironment                = 'Sandbox'
        e03_downgrade_to_1wk_renewal_info.rawExpirationIntent           = None
        e03_downgrade_to_1wk_renewal_info.rawOfferDiscountType          = None
        e03_downgrade_to_1wk_renewal_info.rawOfferType                  = None
        e03_downgrade_to_1wk_renewal_info.rawPriceIncreaseStatus        = None
        e03_downgrade_to_1wk_renewal_info.recentSubscriptionStartDate   = None
        e03_downgrade_to_1wk_renewal_info.renewalDate                   = None
        e03_downgrade_to_1wk_renewal_info.renewalPrice                  = None
        e03_downgrade_to_1wk_renewal_info.signedDate                    = 1759727838450

        # NOTE: Signed Transaction Info
        e03_downgrade_to_1wk_tx_info                                    = AppleJWSTransactionDecodedPayload()
        e03_downgrade_to_1wk_tx_info.appAccountToken                    = None
        e03_downgrade_to_1wk_tx_info.appTransactionId                   = '704897469903383919'
        e03_downgrade_to_1wk_tx_info.bundleId                           = 'com.loki-project.loki-messenger'
        e03_downgrade_to_1wk_tx_info.currency                           = 'AUD'
        e03_downgrade_to_1wk_tx_info.environment                        = AppleEnvironment.SANDBOX
        e03_downgrade_to_1wk_tx_info.expiresDate                        = 1759727970000
        e03_downgrade_to_1wk_tx_info.inAppOwnershipType                 = AppleInAppOwnershipType.PURCHASED
        e03_downgrade_to_1wk_tx_info.isUpgraded                         = None
        e03_downgrade_to_1wk_tx_info.offerDiscountType                  = None
        e03_downgrade_to_1wk_tx_info.offerIdentifier                    = None
        e03_downgrade_to_1wk_tx_info.offerPeriod                        = None
        e03_downgrade_to_1wk_tx_info.offerType                          = None
        e03_downgrade_to_1wk_tx_info.originalPurchaseDate               = 1759301833000
        e03_downgrade_to_1wk_tx_info.originalTransactionId              = '2000001024993299'
        e03_downgrade_to_1wk_tx_info.price                              = 1990
        e03_downgrade_to_1wk_tx_info.productId                          = 'com.getsession.org.pro_sub'
        e03_downgrade_to_1wk_tx_info.purchaseDate                       = 1759727790000
        e03_downgrade_to_1wk_tx_info.quantity                           = 1
        e03_downgrade_to_1wk_tx_info.rawEnvironment                     = 'Sandbox'
        e03_downgrade_to_1wk_tx_info.rawInAppOwnershipType              = 'PURCHASED'
        e03_downgrade_to_1wk_tx_info.rawOfferDiscountType               = None
        e03_downgrade_to_1wk_tx_info.rawOfferType                       = None
        e03_downgrade_to_1wk_tx_info.rawRevocationReason                = None
        e03_downgrade_to_1wk_tx_info.rawTransactionReason               = 'PURCHASE'
        e03_downgrade_to_1wk_tx_info.rawType                            = 'Auto-Renewable Subscription'
        e03_downgrade_to_1wk_tx_info.revocationDate                     = None
        e03_downgrade_to_1wk_tx_info.revocationReason                   = None
        e03_downgrade_to_1wk_tx_info.signedDate                         = 1759727838450
        e03_downgrade_to_1wk_tx_info.storefront                         = 'AUS'
        e03_downgrade_to_1wk_tx_info.storefrontId                       = '143460'
        e03_downgrade_to_1wk_tx_info.subscriptionGroupIdentifier        = '21752814'
        e03_downgrade_to_1wk_tx_info.transactionId                      = '2000001027701928'
        e03_downgrade_to_1wk_tx_info.transactionReason                  = AppleTransactionReason.PURCHASE
        e03_downgrade_to_1wk_tx_info.type                               = AppleType.AUTO_RENEWABLE_SUBSCRIPTION
        e03_downgrade_to_1wk_tx_info.webOrderLineItemId                 = '2000000114202140'

        e03_downgrade_to_1wk_decoded_notification                       = platform_apple.DecodedNotification(body=e03_downgrade_to_1wk_body, tx_info=e03_downgrade_to_1wk_tx_info, renewal_info=e03_downgrade_to_1wk_renewal_info)

        # NOTE: Generated by dump_apple_signed_payloads
        # NOTE: Signed Payload
        e04_revert_to_3months_body                                      = AppleResponseBodyV2DecodedPayload()
        e04_revert_to_3months_body_data                                 = AppleData()
        e04_revert_to_3months_body_data.appAppleId                      = 1470168868
        e04_revert_to_3months_body_data.bundleId                        = 'com.loki-project.loki-messenger'
        e04_revert_to_3months_body_data.bundleVersion                   = '637'
        e04_revert_to_3months_body_data.consumptionRequestReason        = None
        e04_revert_to_3months_body_data.environment                     = AppleEnvironment.SANDBOX
        e04_revert_to_3months_body_data.rawConsumptionRequestReason     = None
        e04_revert_to_3months_body_data.rawEnvironment                  = 'Sandbox'
        e04_revert_to_3months_body_data.rawStatus                       = 1
        e04_revert_to_3months_body_data.signedRenewalInfo               = '..TISJVF_0VsKDc7UAwT3Z4vM8oSRco0cPGQ4z6r3kAi44Ya1f1Rlppfcy49QrnhZGqubPusYnBVG8fMUCXiX3jw'
        ...
        e04_revert_to_3months_body_data.status                          = AppleStatus.ACTIVE
        e04_revert_to_3months_body.data                                 = e04_revert_to_3months_body_data
        e04_revert_to_3months_body.externalPurchaseToken                = None
        e04_revert_to_3months_body.notificationType                     = AppleNotificationTypeV2.DID_CHANGE_RENEWAL_PREF
        e04_revert_to_3months_body.notificationUUID                     = 'a7a530f8-2e03-4b16-ba4c-cee50725f8d8'
        e04_revert_to_3months_body.rawNotificationType                  = 'DID_CHANGE_RENEWAL_PREF'
        e04_revert_to_3months_body.rawSubtype                           = None
        e04_revert_to_3months_body.signedDate                           = 1759727866231
        e04_revert_to_3months_body.subtype                              = None
        e04_revert_to_3months_body.summary                              = None
        e04_revert_to_3months_body.version                              = '2.0'

        # NOTE: Signed Renewal Info
        e04_revert_to_3months_renewal_info                              = AppleJWSRenewalInfoDecodedPayload()
        e04_revert_to_3months_renewal_info.appAccountToken              = None
        e04_revert_to_3months_renewal_info.appTransactionId             = '704897469903383919'
        e04_revert_to_3months_renewal_info.autoRenewProductId           = None
        e04_revert_to_3months_renewal_info.autoRenewStatus              = None
        e04_revert_to_3months_renewal_info.currency                     = 'AUD'
        e04_revert_to_3months_renewal_info.eligibleWinBackOfferIds      = None
        e04_revert_to_3months_renewal_info.environment                  = AppleEnvironment.SANDBOX
        e04_revert_to_3months_renewal_info.expirationIntent             = None
        e04_revert_to_3months_renewal_info.gracePeriodExpiresDate       = None
        e04_revert_to_3months_renewal_info.isInBillingRetryPeriod       = None
        e04_revert_to_3months_renewal_info.offerDiscountType            = None
        e04_revert_to_3months_renewal_info.offerIdentifier              = None
        e04_revert_to_3months_renewal_info.offerPeriod                  = None
        e04_revert_to_3months_renewal_info.offerType                    = None
        e04_revert_to_3months_renewal_info.originalTransactionId        = '2000001024993299'
        e04_revert_to_3months_renewal_info.priceIncreaseStatus          = None
        e04_revert_to_3months_renewal_info.productId                    = 'com.getsession.org.pro_sub'
        e04_revert_to_3months_renewal_info.rawAutoRenewStatus           = None
        e04_revert_to_3months_renewal_info.rawEnvironment               = 'Sandbox'
        e04_revert_to_3months_renewal_info.rawExpirationIntent          = None
        e04_revert_to_3months_renewal_info.rawOfferDiscountType         = None
        e04_revert_to_3months_renewal_info.rawOfferType                 = None
        e04_revert_to_3months_renewal_info.rawPriceIncreaseStatus       = None
        e04_revert_to_3months_renewal_info.recentSubscriptionStartDate  = None
        e04_revert_to_3months_renewal_info.renewalDate                  = None
        e04_revert_to_3months_renewal_info.renewalPrice                 = None
        e04_revert_to_3months_renewal_info.signedDate                   = 1759727866231

        # NOTE: Signed Transaction Info
        e04_revert_to_3months_tx_info                                   = AppleJWSTransactionDecodedPayload()
        e04_revert_to_3months_tx_info.appAccountToken                   = None
        e04_revert_to_3months_tx_info.appTransactionId                  = '704897469903383919'
        e04_revert_to_3months_tx_info.bundleId                          = 'com.loki-project.loki-messenger'
        e04_revert_to_3months_tx_info.currency                          = 'AUD'
        e04_revert_to_3months_tx_info.environment                       = AppleEnvironment.SANDBOX
        e04_revert_to_3months_tx_info.expiresDate                       = 1759727970000
        e04_revert_to_3months_tx_info.inAppOwnershipType                = AppleInAppOwnershipType.PURCHASED
        e04_revert_to_3months_tx_info.isUpgraded                        = None
        e04_revert_to_3months_tx_info.offerDiscountType                 = None
        e04_revert_to_3months_tx_info.offerIdentifier                   = None
        e04_revert_to_3months_tx_info.offerPeriod                       = None
        e04_revert_to_3months_tx_info.offerType                         = None
        e04_revert_to_3months_tx_info.originalPurchaseDate              = 1759301833000
        e04_revert_to_3months_tx_info.originalTransactionId             = '2000001024993299'
        e04_revert_to_3months_tx_info.price                             = 1990
        e04_revert_to_3months_tx_info.productId                         = 'com.getsession.org.pro_sub'
        e04_revert_to_3months_tx_info.purchaseDate                      = 1759727790000
        e04_revert_to_3months_tx_info.quantity                          = 1
        e04_revert_to_3months_tx_info.rawEnvironment                    = 'Sandbox'
        e04_revert_to_3months_tx_info.rawInAppOwnershipType             = 'PURCHASED'
        e04_revert_to_3months_tx_info.rawOfferDiscountType              = None
        e04_revert_to_3months_tx_info.rawOfferType                      = None
        e04_revert_to_3months_tx_info.rawRevocationReason               = None
        e04_revert_to_3months_tx_info.rawTransactionReason              = 'PURCHASE'
        e04_revert_to_3months_tx_info.rawType                           = 'Auto-Renewable Subscription'
        e04_revert_to_3months_tx_info.revocationDate                    = None
        e04_revert_to_3months_tx_info.revocationReason                  = None
        e04_revert_to_3months_tx_info.signedDate                        = 1759727866231
        e04_revert_to_3months_tx_info.storefront                        = 'AUS'
        e04_revert_to_3months_tx_info.storefrontId                      = '143460'
        e04_revert_to_3months_tx_info.subscriptionGroupIdentifier       = '21752814'
        e04_revert_to_3months_tx_info.transactionId                     = '2000001027701928'
        e04_revert_to_3months_tx_info.transactionReason                 = AppleTransactionReason.PURCHASE
        e04_revert_to_3months_tx_info.type                              = AppleType.AUTO_RENEWABLE_SUBSCRIPTION
        e04_revert_to_3months_tx_info.webOrderLineItemId                = '2000000114202140'

        e04_revert_to_3months_decoded_notification                      = platform_apple.DecodedNotification(body=e04_revert_to_3months_body, tx_info=e04_revert_to_3months_tx_info, renewal_info=e04_revert_to_3months_renewal_info)
        ...
        err = base.ErrorSink()
        platform_apple.handle_notification(decoded_notification = e00_sub_to_1wk_decoded_notification,         sql_conn=db.sql_conn, err=err)
        platform_apple.handle_notification(decoded_notification = e01_upgrade_to_3months_decoded_notification, sql_conn=db.sql_conn, err=err)
        platform_apple.handle_notification(decoded_notification = e02_disable_auto_renew_decoded_notification, sql_conn=db.sql_conn, err=err)
        platform_apple.handle_notification(decoded_notification = e03_downgrade_to_1wk_decoded_notification,   sql_conn=db.sql_conn, err=err)
        platform_apple.handle_notification(decoded_notification = e04_revert_to_3months_decoded_notification,  sql_conn=db.sql_conn, err=err)
        platform_apple.handle_notification(decoded_notification = e05_disable_auto_renew_decoded_notification, sql_conn=db.sql_conn, err=err)
        platform_apple.handle_notification(decoded_notification = e06_expire_voluntary_decoded_notification,   sql_conn=db.sql_conn, err=err)
        assert not err.has(), err.msg_list
...
{'kind': 'androidpublisher#subscriptionPurchaseV2', 'startTime': '2025-10-06T03:58:10.981Z', 'regionCode': 'AU', 'subscriptionState': 'SUBSCRIPTION_STATE_EXPIRED', 'latestOrderId': 'GPA.3354-3745-5570-25336', 'canceledStateContext': {'systemInitiatedCancellation': {}}, 'testPurchase': {}, 'acknowledgementState': 'ACKNOWLEDGEMENT_STATE_PENDING', 'lineItems': [{'productId': 'session_pro', 'expiryTime': '2025-10-06T04:03:11.808Z', 'autoRenewingPlan': {'recurringPrice': {'currencyCode': 'AUD', 'units': '16', 'nanos': 990000000}}, 'offerDetails': {'basePlanId': 'session-pro-1-month', 'offerTags': ['one-month']}, 'latestSuccessfulOrderId': 'GPA.3354-3745-5570-25336'}]})
    ...
    assert item_status == backend.PaymentStatus.Expired
