...
def dump_apple_signed_payloads(core: platform_apple.Core, body: AppleResponseBodyV2DecodedPayload):
    ...
    print(print_python_decl_code_for_apple_obj(body, 0, "body"))

    print("# NOTE: Signed Renewal Info")
    print(print_python_decl_code_for_apple_obj(platform_apple.maybe_get_apple_jws_renewal_info_from_response_body_v2(body, core.signed_data_verifier, None), 0, 'renewal_info'))

    print("# NOTE: Signed Transaction Info")
    print(print_python_decl_code_for_apple_obj(platform_apple.maybe_get_apple_jws_transaction_from_response_body_v2(body, core.signed_data_verifier, None), 0, 'tx_info') + "\n")

    print('decoded_notification = platform_apple.DecodedNotification(body=body, tx_info=tx_info, renewal_info=renewal_info)')
    print('platform_apple.handle_notification(decoded_notification=decoded_notification, sql_conn=db.sql_conn)')
...
def test_platform_apple():
    ...
    if 1: # Did renew notification
        ...
        renewal                             = AppleJWSRenewalInfoDecodedPayload()
        renewal.appAccountToken             = None
        renewal.appTransactionId            = '704897469903383919'
        renewal.autoRenewProductId          = None
        renewal.autoRenewStatus             = None
        renewal.currency                    = 'AUD'
        renewal.eligibleWinBackOfferIds     = None
        renewal.environment                 = AppleEnvironment.SANDBOX
        renewal.expirationIntent            = None
        renewal.gracePeriodExpiresDate      = None
        renewal.isInBillingRetryPeriod      = None
        renewal.offerDiscountType           = None
        renewal.offerIdentifier             = None
        renewal.offerPeriod                 = None
        renewal.offerType                   = None
        renewal.originalTransactionId       = '2000001024993299'
        renewal.priceIncreaseStatus         = None
        renewal.productId                   = 'com.getsession.org.pro_sub'
        renewal.rawAutoRenewStatus          = None
        renewal.rawEnvironment              = 'Sandbox'
        renewal.rawExpirationIntent         = None
        renewal.rawOfferDiscountType        = None
        renewal.rawOfferType                = None
        renewal.rawPriceIncreaseStatus      = None
        renewal.recentSubscriptionStartDate = None
        renewal.renewalDate                 = None
        renewal.renewalPrice                = None
        renewal.signedDate                  = 1759302518835

        tx                                  = AppleJWSTransactionDecodedPayload()
        tx.appAccountToken                  = None
        tx.appTransactionId                 = '704897469903383919'
        tx.bundleId                         = 'com.loki-project.loki-messenger'
        tx.currency                         = 'AUD'
        tx.environment                      = AppleEnvironment.SANDBOX
        tx.expiresDate                      = 1759302732000
        tx.inAppOwnershipType               = AppleInAppOwnershipType.PURCHASED
        tx.isUpgraded                       = None
        tx.offerDiscountType                = None
        tx.offerIdentifier                  = None
        tx.offerPeriod                      = None
        tx.offerType                        = None
        tx.originalPurchaseDate             = 1759301833000
        tx.originalTransactionId            = '2000001024993299'
        tx.price                            = 1990
        tx.productId                        = 'com.getsession.org.pro_sub'
        tx.purchaseDate                     = 1759302552000
        tx.quantity                         = 1
        tx.rawEnvironment                   = 'Sandbox'
        tx.rawInAppOwnershipType            = 'PURCHASED'
        tx.rawOfferDiscountType             = None
        tx.rawOfferType                     = None
        tx.rawRevocationReason              = None
        tx.rawTransactionReason             = 'RENEWAL'
        tx.rawType                          = 'Auto-Renewable Subscription'
        tx.revocationDate                   = None
        tx.revocationReason                 = None
        tx.signedDate                       = 1759302518835
        tx.storefront                       = 'AUS'
        tx.storefrontId                     = '143460'
        tx.subscriptionGroupIdentifier      = '21752814'
        tx.transactionId                    = '2000001024998954'
        tx.transactionReason                = AppleTransactionReason.RENEWAL
        tx.type                             = AppleType.AUTO_RENEWABLE_SUBSCRIPTION
        tx.webOrderLineItemId               = '2000000113755461'

        notification = platform_apple.DecodedNotification(body=body, tx_info=tx, renewal_info=renewal)
