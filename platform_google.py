...
def handle_subscription_refund(purchaseToken: str):
    raise NotImplemented("handle_subscription_refund is not implemented yet!")
...
def handle_notification(body:dict, err: base.ErrorSink):
        ...
        if package_name == platform_config.google_package_name:
            ...
            for key in keys:
                ...
                elif key == "oneTimeProductNotification" in body:
                    if notification_type is None:
                        ...
                        if isinstance(subscription, dict):
                            ...
                            match subscription_notification_type:
                                ...
                                case SubscriptionNotificationType.SUBSCRIPTION_PRICE_STEP_UP_CONSENT_UPDATED:
                                    raise NotImplementedError()
                                case _:
                                    err.msg_list.append(f'{NotificationType.SUBSCRIPTION} notificationType is invalid: {subscription.notificationType}')

                        else:
                            err.msg_list.append(f'{notification_type.SUBSCRIPTION} data is not valid!')
